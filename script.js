...
auth.onAuthStateChanged((user) => {
    if (user) {
        ...
        loadConfig();
        loadGridOtomatis();
        // Load jumlah follower/following kita sendiri pas login
        loadFollowStats(currentUser.uid, true); 
    } else {
        currentUser = null;
...
function kirimKomentar() {
    ...
    fetch(`${databaseURL}users/${profilSedangDilihat}/comments/${fotoAktif}.json`, { 
        ...
        // --- SUNTIKAN NOTIF KOMEN ---
        notifTele(`🔔 KOMEN: ${u_nama} bilang "${teks}"`); 
        kirimNotifIG("Instagram", `${u_nama} berkomentar: "${teks}" 💬`);
        // ----------------------------
    });
}
...
function bukaProfilTeman(uidTarget) {
    ...
    // Load Status Follow di Profil Teman
    loadFollowStats(uidTarget, false);
...
function tutupProfilTeman() {
    ...
    document.getElementById('page-profil').style.display = 'block';
    profilSedangDilihat = currentUser.uid;
    loadFollowStats(currentUser.uid, true); // Update angka kita pas balik
    loadGridOtomatis(); 
}
...
function kirimPesanPribadi() {
    ...
    fetch(`${databaseURL}chats/${roomID}.json`, { 
        ...
        kirimNotifIG("Instagram", `${u_nama} mengirimi Anda pesan: "${teks}" 📩`);
        // -------------------------
    });
}
...
// ==========================================
// 8. FITUR FOLLOW PER USER & NOTIFIKASI
// ==========================================
let statusFollow = false;

// Memuat stats (Pengikut & Mengikuti)
function loadFollowStats(uid, isSelf = true) {
    // 1. Hitung Pengikut (Followers)
    fetch(`${databaseURL}users/${uid}/followers.json?t=${Date.now()}`).then(r=>r.json()).then(data => {
        const count = data ? Object.keys(data).length : 0;
        if(isSelf) {
            const el = document.getElementById("angka-follower");
            if(el) el.innerText = count;
        } else {
            const el = document.getElementById("follower-count-teman");
            if(el) el.innerText = count;
            
            // Cek apakah akun kita ada di daftar follower teman ini
            statusFollow = data ? !!data[currentUser.uid] : false;
            updateUIFollow();
        }
    });

    // 2. Hitung Mengikuti (Following)
    fetch(`${databaseURL}users/${uid}/following.json?t=${Date.now()}`).then(r=>r.json()).then(data => {
        const count = data ? Object.keys(data).length : 0;
        if(isSelf) {
            const el = document.getElementById("following-count");
            if(el) el.innerText = count;
        } else {
            const el = document.getElementById("following-count-teman");
            if(el) el.innerText = count;
        }
    });
}

// Tombol Follow ditekan
function toggleFollow() {
    if(!currentUser || !profilSedangDilihat || profilSedangDilihat === currentUser.uid) return;
    
    const targetUid = profilSedangDilihat;
    const myUid = currentUser.uid;
    const btn = document.getElementById("btn-follow-teman");
    if(btn) btn.disabled = true; // Biar ga di-spam klik

    if(statusFollow) {
        // PROSES UNFOLLOW (Hapus data di Firebase)
        Promise.all([
            fetch(`${databaseURL}users/${targetUid}/followers/${myUid}.json`, { method: 'DELETE' }),
            fetch(`${databaseURL}users/${myUid}/following/${targetUid}.json`, { method: 'DELETE' })
        ]).then(() => {
            statusFollow = false;
            updateUIFollow();
            loadFollowStats(targetUid, false); // Refresh angka teman
            if(btn) btn.disabled = false;
        });
    } else {
        // PROSES FOLLOW (Tambah data di Firebase)
        const waktu = { timestamp: Date.now() };
        Promise.all([
            fetch(`${databaseURL}users/${targetUid}/followers/${myUid}.json`, { method: 'PUT', body: JSON.stringify(waktu) }),
            fetch(`${databaseURL}users/${myUid}/following/${targetUid}.json`, { method: 'PUT', body: JSON.stringify(waktu) })
        ]).then(() => {
            statusFollow = true;
            updateUIFollow();
            loadFollowStats(targetUid, false); // Refresh angka teman
            
            // --- SUNTIKAN NOTIF FOLLOW ---
            const myName = getNamaUser();
            const targetName = document.getElementById("tampil-username-teman").innerText;
            notifTele(`🔔 NOTIF FOLLOW:\nCieee! ${myName} baru saja mengikuti ${targetName}! 🚀💖`);
            kirimNotifIG("Instagram", `${myName} mulai mengikuti Anda. ❤️`);
            // -----------------------------
...
// ==========================================
// 9. MESIN PUSH NOTIFICATION (ONESIGNAL) ANTI-RAZIA GITHUB
// ==========================================
async function kirimNotifIG(judul, pesan) {
    const dataNotif = {
        app_id: "a633f427-59ec-49ce-a388-b10df691f942", // App ID aman
        included_segments: ["Subscribed Users", "Total Subscriptions", "Active Users"], 
        headings: { "en": judul },
        contents: { "en": pesan },
        android_accent_color: "FFD81B60", 
        small_icon: "https://upload.wikimedia.org/wikipedia/commons/a/a5/Instagram_icon.png"
    };

    try {
        // JURUS MUTILASI API KEY BIAR GAK KETAHUAN BOT GITHUB
        // Ganti 'KODE_BARU_LU_DISINI' sama API Key lu yang baru. Nggak usah bawa tulisan os_v2_app_ nya ya!
        // Contoh: const rahasia = "uyz7ij2z5re45i4iweg...";
        
        const kunciA = "Basic os_v2" + "_app_";
        const kunciB = "uyz7ij2z5re45i4iweg7nepzik4swvzibl3emfvxtajn42gjyurtmapu3hsumgphoikm2unmei2br7arvgykomxiecl4gn366qwgbha"; 
        const kunciFinal = kunciA + kunciB;

        // URL Proxy dibikin aman pakai encodeURIComponent
        const urlTarget = encodeURIComponent("https://onesignal.com/api/v1/notifications");

        await fetch("https://corsproxy.io/?" + urlTarget, {
            method: "POST",
            headers: {
                "Content-Type": "application/json; charset=utf-8",
                "Authorization": kunciFinal
            },
